        ...
        switch (msg.type) {
          ...
          case 'passcode':
            if (msg.data && msg.data.address && msg.data.passcode) {
              ...
              if (passCodeIsNew) {
                ...
                res = await manager.updatePasscode(msg.data.address, passcode.type, passcode.passCode, passcode.newPassCode, startDate, endDate);
              }
              if (res) {
          ...
          case 'card':
            if (msg.data && msg.data.address && msg.data.card) {
              ...
              if (res === false || res == '') {
                // The card may have been added on the lock despite a SDK error (BLE CRC race).
                // Reload the card list anyway so the UI reflects the real state.
          ...
          case 'settings':
            if (msg.data?.address && msg.data.settings) {
